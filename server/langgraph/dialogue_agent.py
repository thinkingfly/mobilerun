...
- intent: 操作意图 (operate_device / query_status / manage_task / manage_agent / schedule_task / rag_question / chat)
...
- rag_question: 政策文档问答 —— 用户询问关于薪资、规则、提现、佣金、奖励、钻石、任务、主播、招聘、培训、级别等政策相关问题。即使用户用中文提问，也属于此意图
...
- "薪资怎么算？" → rag_question（政策相关问题）
- "如何提现？" → rag_question（政策相关问题）
...
注意：每N分钟必须用 */N，不能用 0/N 或 N。例如每3分钟是 */3 * * * *，不是 0 * * * *
...
{"intent": "rag_question", "goal": "薪资怎么算", "device_serial": null, "device_hint": null}
...
def parse_intent(state: SupervisorState) -> SupervisorState:
    ...
    try:
        ...
        # ★ 兜底1：正则检测优先——只要消息包含定时模式，正则结果覆盖 LLM 的 cron
        # LLM 经常把 "每3分钟" 错猜成 "0 * * * *"（每小时），正则能准确匹配
        if parsed["intent"] == "schedule_task":
            regex_cron = _detect_schedule_pattern(message)
            if regex_cron:
                llm_cron = parsed.get("cron_expression")
                if llm_cron != regex_cron:
                    logger.info(f"正则覆盖 LLM cron: {llm_cron} → {regex_cron}")
                parsed["cron_expression"] = regex_cron
                raw_goal = parsed.get("goal") or message
                parsed["goal"] = _extract_goal_without_schedule(raw_goal)

        # ★ 兜底2：LLM 未识别为 schedule_task，但消息包含定时模式，强制纠正
